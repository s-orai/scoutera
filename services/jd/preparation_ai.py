...
def format_prompt_for_business_description(company_info):
  return f"""
            ...
            ・会社の正式名称を出力する
            ## 事業サービス名
            ・特定の自社サービスを持っている場合は、サービス名を出力する
             - 複数ある場合は、すべて出力する
            ・SIerやコンサル企業など特定の自社サービス名がない場合は、サービス名ではなく事業業種/ソリューション名を出力する
             - 例：採用コンサルティング事業, SI・システム開発事業など
            ## 企業理念
            ### 構成
            ...
            ・今後の展開
            ### 作成時の制約条件・注意事項
            ・事業が複数ある場合は、作成例を参考にそれぞれについて記述を作成する
              - 事業名とそれについての詳細を250字以内で作成する
              - 全ての事業について、それぞれ作成する
              - 事業ごとに改行すること
            ・事業が1つのみの場合は、その詳細を400字以内で作成する
            ・業界外の人でも理解できるよう、専門用語は避ける、もしくは補足を入れる
...
def format_prompt_for_jd(company_info, hearing_info, jd_title):
  return f"""
            ...
            ## 募集職種
            ### 構成
            ・該当ポジションの職種名を1つのみ出力する
            ### 作成時の制約条件・注意事項
            ・世間一般的に通用する職種名で出力すること
              - 領域が特化されている場合でも、一般的な職種名で出力する
              - (例) 宇宙領域特化のプロダクトマネージャー→「プロダクトマネージャー」で出力
            ・同じ職種で業務内容が幅広い場合は、どの分野の職種なのかがわかるように出力する
              - (例)「エンジニア」では幅広すぎるので「フルスタックエンジニア」「QAエンジニア」「機械学習エンジニア」など、どの分野のエンジニアなのかわかるように出力する
            ## 必須要件
            ### 構成
